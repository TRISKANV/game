 ...
 * WeaponSystem
 *
 * Fix: all enemy/projectile/render/health iterations use World snapshot
 * methods to prevent ConcurrentModificationException. Entities created
 * or destroyed inside a loop never affect the snapshot being iterated.
 */
class WeaponSystem {
    ...
    fun update(world: World, dt: Float) {
        ...
        for (weapon in weapons) {
            ...
            when (weapon.type) {
                ...
                WeaponType.GARLIC      -> { /* handled by aura update below */ }
                ...
                WeaponType.BIBLE       -> { /* handled by orbital update below */ }
            }
        }
    ...
    private fun fireMagicWand(
        ...
        repeat(weapon.projectileCount) { i ->
            ...
            EntityFactory.createProjectile(world, ox, oy, cos(angle + off) * speed, sin(angle + off) * speed,
                WeaponType.MAGIC_WAND, dmg, size, weapon.piercing, dur, crit, weapon.knockback)
        }
    }
    ...
    private fun fireKnife(
        ...
        repeat(weapon.projectileCount) { i ->
            ...
            EntityFactory.createProjectile(world, ox, oy,
                ...
                WeaponType.KNIFE, dmg, size * 0.7f, weapon.piercing, dur * 0.6f, crit, weapon.knockback * 0.5f)
        }
    }
        ...
        for (angle in directions) {
            repeat(weapon.projectileCount) {
                EntityFactory.createProjectile(world, ox, oy, cos(angle) * speed, sin(angle) * speed,
                    WeaponType.CROSS, dmg, size, weapon.piercing, dur, crit, weapon.knockback)
            }
        }
    }
    ...
    private fun fireFireWand(
        ...
        repeat(weapon.projectileCount) { i ->
            ...
            EntityFactory.createProjectile(world, ox, oy, cos(angle + off) * speed, sin(angle + off) * speed,
                WeaponType.FIRE_WAND, dmg, size, weapon.piercing + 2, dur, crit, 0f)
        }
    }
    ...
    private fun fireAxe(
        ...
        repeat(weapon.projectileCount) { i ->
            ...
            EntityFactory.createProjectile(world, ox, oy,
                ...
                WeaponType.AXE, dmg, size * 1.4f, weapon.piercing, dur * 1.2f, crit, weapon.knockback * 1.5f)
        }
    }
    ...
    private fun fireLightning(
        ...
        val range = 350f * area
        // SNAPSHOT before sorting/iterating
        val enemies = findEnemiesInRadius(world, ox, oy, range)
            .sortedBy { eid ->
                val t = world.transforms[eid]
                if (t != null) { val dx = t.x - ox; val dy = t.y - oy; dx * dx + dy * dy }
    ...
    private fun fireWhip(
        ...
        val halfW = 120f * area
        val halfH = 40f  * area
        // SNAPSHOT — fireWhip marks health.isDead which doesn't structurally
        // modify enemies map, but we snapshot anyway for consistency
    ...
    private fun updateProjectileLifetimes(world: World, dt: Float) {
        ...
        // SNAPSHOT — iterating then calling destroyEntity would CME on live map
        for ((id, proj) in world.getProjectileMapSnapshot()) {
            proj.lifetime -= dt
            if (proj.lifetime <= 0f) {
    ...
    private fun updateRenderTimers(world: World, dt: Float) {
        // SNAPSHOT — no structural mutation, but consistent with pattern
        for ((_, render) in world.getRenderMapSnapshot()) {
    ...
    private fun updateEnemyIFrames(world: World, dt: Float) {
        // SNAPSHOT
        for ((_, health) in world.getHealthMapSnapshot()) {
